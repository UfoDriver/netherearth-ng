		...
		if (b.pos.y>=(viewp.y+MINY) &&
			...
			case Building::B_FACTORY_ELECTRONICS:
						 ...
						 if (b.owner==1) {
							if (!shadows) {
								glTranslatef(0,-1,1);
								building_tiles[6].draw(Color(0.3f, 0.3f, 0.8f));
							} else {
								glTranslatef(float(-light.x),float(-light.y)-1,0.05f);
								building_tiles[6].DrawShadow(Color(0, 0, 0, 0.5));
							} /* if */ 
						 } /* if */ 
						 if (b.owner==2) {
							if (!shadows) {
								glTranslatef(0,-1,1);
								building_tiles[6].draw(Color(0.8f, 0.3f, 0.3f));
							} else {
								glTranslatef(float(-light.x),float(-light.y)-1,0.05f);
								building_tiles[6].DrawShadow(Color(0, 0, 0, 0.5));
							} /* if */ 
			...
			case Building::B_WARBASE:if (!shadows) {
                           if (detaillevel>=2) building_tiles[8].draw(Color(0.5, 0.5, 0.5));
							   ...
							   glTranslatef(0,0,0.05f);
							   building_tiles[8].DrawShadow(Color(0, 0, 0, 0.5));
						   } /* if */ 
